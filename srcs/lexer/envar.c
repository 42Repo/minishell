...
/*   Updated: 2024/05/09 23:33:04 by mbuchs           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*replace_envar(t_data *data, t_token *selected, int i)
{
	...
	while(selected->value[i])
	{
		...
		while (selected->value[i] && !((int)ft_strlen(selected->value) >= i + 1 && \
				selected->value[i] == '$' \
				&& (ft_isalpha(selected->value[i + 1]) || selected->value[i + 1] == '_' || selected->value[i + 1] == '?' \
		...
		if ((quote_state == 2 && selected->value[i + 1] == '"') || (quote_state == 1 && selected->value[i + 1] == '\''))
		{
			quote_state = 0;
			i++;
			j++;
